...
class JniParams(object):
  ...
  def Parse(params):
    ...
    if not params:
      return []
    ret = []
    for p in [p.strip() for p in JniParams.SplitParams(params)]:
      items = p.split(' ')
      if 'final' in items:
        items.remove('final')
      param = Param(
  ...
  def SplitParams(params):
    if not params or (params.find(',') == -1):
      return [params]
    ...
    bracket_num_dev = 0
    result_string = ''
    for sub_string in param_list:
      bracket_num_dev = bracket_num_dev + sub_string.count('<') - sub_string.count('>')
      result_string = result_string + sub_string.strip()
      if bracket_num_dev == 0:
        result_params.append(result_string)
        result_string = ''
      else:
...
def ExtractJNINamespace(contents):
  re_jni_namespace = re.compile(r'.*?@JNINamespace\("(.*?)"\)')
...
# ScopedLocalJavaRef.
RE_SCOPED_JNI_RETURN_TYPES = re.compile(r'jobject|jclass|jstring|.*Array')
...
class JNIFromJavaP(object):
  ...
  def __init__(self, contents, options):
    ...
    for line in contents:
      class_name = re.match(
          r'.*?(public).*?(class|interface) (?P<class_name>\S+?)( |\Z)',
          line)
      if class_name:
    ...
    re_method = re.compile(r'(?P<prefix>.*?)(?P<return_type>\S+?) (?P<name>\w+?)'
                           r'\((?P<params>.*?)\)')
    ...
    re_constructor = re.compile(r'(.*?)public ' +
    ...
    re_constant_field = re.compile(r'.*?public static final int (?P<name>.*?);')
    re_constant_field_value = re.compile(
        r'.*?Constant(Value| value): int (?P<value>(-*[0-9]+)?)')
    for lineno, content in enumerate(contents[2:], 2):
...
""")
    values = {
        ...
        'JNI_UTILS_HEADER': self.GetJniUtilsHeaderString()
    }
...
""")
    ...
    if self.options.native_exports_optional:
      early_exit = """\
  if (lynx::base::android::IsManualJniRegistrationDisabled()) return true;
"""
...
""")
  ...
  def GetJNIRegisterNativesString(self):
    ...
    template = Template("""\
extern "C" JNIEXPORT ALLOW_UNUSED_TYPE bool JNICALL
...
""")
  ...
  def GetJniUtilsHeaderString(self):
    if self.options.use_base_jni_utils_header:
      return '"base/include/platform/android/jni_utils.h"'
    else:
      return '"core/base/android/android_jni.h"'
  ...
  def GetForwardDeclaration(self, native):
    template_str = """
${RETURN} ${NAME}(JNIEnv* env, ${PARAMS});
"""
    if not self.namespace:
      template_str = 'static ' + template_str
    if self.options.native_exports:
      template_str += """
...
"""
  ...
  def GetCalledByNativeValues(self, called_by_native):
    ...
    if not called_by_native.unchecked:
      if self.options.use_base_jni_utils_header: 
        check_exception = 'lynx::base::android::CheckAndPrintException(env);'
      else:
        check_exception = 'lynx::base::android::CheckException(env);'
    ...
    if return_type != 'void':
      ...
      if re.match(RE_SCOPED_JNI_RETURN_TYPES, return_type):
        return_type = 'lynx::base::android::ScopedLocalJavaRef<' + return_type + '>'
  ...
  def GetEagerCalledByNativeMethodStub(self, called_by_native):
    ...
    template = Template("""
static ALLOW_UNUSED_TYPE ${RETURN_TYPE} ${METHOD_ID_VAR_NAME}(\
...
static ALLOW_UNUSED_TYPE ${RETURN_TYPE} Java_${JAVA_CLASS}_${METHOD_ID_VAR_NAME}(\
JNIEnv* env${FIRST_PARAM_IN_DECLARATION}${PARAMS_IN_DECLARATION})""")
...
lynx::base::subtle::AtomicWord g_${JAVA_CLASS}_clazz __attribute__((unused)) = 0;
#define ${JAVA_CLASS}_clazz(env) \
lynx::base::android::LazyGetClass(env, k${JAVA_CLASS}ClassPath, \
...
#define ${JAVA_CLASS}_clazz(env) g_${JAVA_CLASS}_clazz""")
  ...
  def GetFindClasses(self):
    ...
    if self.init_native:
      if self.options.native_exports:
        template = Template("""\
  lynx::base::subtle::Release_Store(&g_${JAVA_CLASS}_clazz,
      static_cast<lynx::base::subtle::AtomicWord>(env->NewWeakGlobalRef(clazz));""")
      else:
        template = Template("""\
  ...
  g_${JAVA_CLASS}_clazz = reinterpret_cast<jclass>(env->NewGlobalRef(
      lynx::base::android::GetClass(env, k${JAVA_CLASS}ClassPath).Get()));""")
    ret = []
...
env->Get${STATIC_METHOD_PART}MethodID(
  ...
  lynx::base::android::GetMethod(
      ...
      lynx::base::android::${STATIC}_METHOD,
      "${JNI_NAME}",
      ${JNI_SIGNATURE},
...
class Options(object):
  ...
  def __init__(self, use_base_jni_utils_header):
    self.ptr_type = 'int'
...
def parse_args(args):
  ...
  parser.add_argument('--use-base-jni-header', dest='use_base_jni_utils_header', action='store_true', default=False)
...
if __name__ == '__main__':
  ...
  if(len(sys.argv) >= 3):
    ...
    options = Options(args.use_base_jni_utils_header)
